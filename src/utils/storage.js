...
export default {
	...
	saveFile(file) {
		...
		if (FileUtil.fileExists(`${StoragePath}/${filename}`)) {
			// if so, generate a new name
			filename = `${filename.split(".")[0]}_${Date.now()}.${
				filename.split(".")[1]
			}`;
		}
		...
		FileUtil.writeToFile(absolutePath, file.buffer);
	...
	getFile(filename) {
		...
		return new Asset(
			filename,
			absolutePath,
			FileUtil.readFromFile(absolutePath)
		);
	},
	...
	removeFile(filename) {
		try {
			FileUtil.removeFile(`${StoragePath}/${filename}`);
			return true;
		} catch (error) {
			return false;
		}
	...
	getFilesByTag(tag) {
		...
		const files = FileUtil.getFilesInDirectory(StoragePath).filter((file) =>
			file.startsWith(`${tag}_`)
		);
		// return assets
